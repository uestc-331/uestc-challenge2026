...
def camera_point_from_detection(
    box: Sequence[int], depth_z: float, intrinsics: Sequence[float], sphere_radius_m: float
) -> np.ndarray:
    ...
    surface = ray * depth_z
    center = surface + sphere_radius_m * ray / np.linalg.norm(ray)
...
def cluster_points(
    ...
    for value in values:
        distances = [
            float(np.linalg.norm(value - np.asarray(cluster).mean(axis=0)))
            for cluster in clusters
        ]
        nearest = int(np.argmin(distances)) if distances else None
        if nearest is not None and distances[nearest] <= radius_m:
    ...
    centers.sort(key=lambda point: tuple(float(value) for value in point))
    return centers
...
class LiveRouteEvaluator:
    ...
    def _image_callback(self, rgb_message, depth_message, camera_info) -> None:
        ...
        for index, prediction in enumerate(predictions):
            ...
            x0, y0, x1, y1 = box
            depth_z = median_depth_in_box(depth, box)
            world_position = None
            if depth_z is not None:
                point_camera = camera_point_from_detection(
                    box, depth_z, intrinsics, self.args.sphere_radius
                )
                point_world = camera_to_world(point_camera, robot_pose, self.camera_offset)
                self.world_points.append(point_world)
            ...
            label = f"red_sphere {confidence:.2f}"
            if depth_z is not None:
    ...
    def finalize(self, route_completed: bool, route_index: int) -> Dict[str, object]:
        ...
        centers = cluster_points(
            self.world_points, self.args.cluster_radius, self.args.min_cluster_observations
        )
        detected_payload = {
        ...
        detail = {
            ...
            "raw_3d_observations": len(self.world_points),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--min-cluster-observations", type=int, default=3)
